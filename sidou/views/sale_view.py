...
class SaleView(QWidget):
    ...
    def init_ui(self):
        ...
        self.price_display_label = QLabel("Prix Unit.: --.-- DA")
        ...
        self.total_label = QLabel("Total Vente: 0.00 DA")
        self.total_label.setStyleSheet("font-weight: bold; font-size: 14pt; color: #2c3e50;")
        
        button_layout = QHBoxLayout()
        self.clear_sale_button = QPushButton("Annuler Vente")
        self.clear_sale_button.setStyleSheet("""
            QPushButton {
        ...
        new_sale_layout.addWidget(separator)
    ...
    def load_customers_for_sale(self):
        ...
        self.customer_combo.addItem("Vente Anonyme", -1)
        try:
        ...
        if selected_index > 0:
            ...
            if product_id in self.products_cache:
                ...
                self.price_display_label.setText(
                    f"Prix Unit.: {product_info['price']:.2f} DA"
                )
                ...
                self.price_display_label.setText("Prix Unit.: --.-- DA")
    ...
    def refresh_current_sale_table(self):
        ...
        for row_idx, item in enumerate(self.current_sale_items):
            ...
            self.current_sale_table.setItem(
                row_idx, 2, QTableWidgetItem(f"{item['price_at_sale']:.2f} DA")
            )
            self.current_sale_table.setItem(
                row_idx, 3, QTableWidgetItem(f"{item['subtotal']:.2f} DA")
            )
    ...
    def update_total(self):
        ...
        self.total_label.setText(f"Total Vente: {total:.2f} DA")
        ...
        try:
            ...
            if history:
                ...
                for row_idx, sale in enumerate(history):
                    ...
                    self.history_table.setItem(
                        row_idx, 3, QTableWidgetItem(f"{sale['total_amount']:.2f} DA")
                    )
        except Exception as e:
            QMessageBox.critical(
                self,
    ...
    def generate_receipt_text(self, sale_id):
        ...
        try:
            ...
            receipt += f"{'-'*40}\n"
            receipt += f"{'MONTANT TOTAL:':>32} {sale_header['total_amount']:.2f} DA\n"
    ...
    def populate_table(self, items_data):
        ...
        if items_data:
            ...
            for row_idx, item in enumerate(items_data):
                ...
                self.details_table.setItem(
                    row_idx, 3, QTableWidgetItem(f"{subtotal:.2f} DA")
                )
